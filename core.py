...
aircraftList = (
    aircraftStatsDf["aircraft"].sort_values().astype(str).values.flatten().tolist()
)
...
requiredRunway = (
    aircraftStatsDf["minRunway"]
    .loc[aircraftStatsDf["aircraft"] == aircraftType]
    .values[0]
)
...
rangeMax = (
    aircraftStatsDf["range"].loc[aircraftStatsDf["aircraft"] == aircraftType].values[0]
)
...
availableFlightsDf = flightsDf[["airport", "flightUrl", "slots", "gatesAvailable"]].loc[
    flightsDf["flightCreated"] == False
]
if not availableFlightsDf.empty:
    ...
    # Add hub
    if autoHub == "y":
        api.add_hub(phpSessidReq, depAirportCode)
