...
def main():
    ...
    filtered_types: list = [
        t for t in all_types if t.attrs.get("value", "").isdigit()]
    ...
    maker_market_buy_page_req, _, _ = getRequest(
        ...
        cookies={"PHPSESSID": session_id},
        params={"maker": 45, "next": -1}
    )
    maker_market_buy_page = BeautifulSoup(
        maker_market_buy_page_req.text, 'lxml')
    table_input: list = maker_market_buy_page.find(
        "form", method="post").find_all("input")
    aircraft_ids: list = [i.attrs.get(
        "value") for i in table_input if i.attrs.get("name") == "aircraftid[]"]
